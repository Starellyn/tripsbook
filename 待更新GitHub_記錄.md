...
- **num 重編**(已裁定):variant 模式下 `num` 隨顯示順序重編為英文序數(DAY ONE…DAY FOURTEEN,超出以數字 fallback);無 variants 時一律顯示作者原值。規格書 26 章補渲染規則、模版改查表實作。
- **景點推薦分頁**(已裁定):整趟層級、全版本共用、不隨 variant 過濾(彙整全部版本的 spot)。規格書 26 章補明文,模版維持原實作。
  註:裁定原文寫「美食分頁」,依回報脈絡判為「景點推薦分頁」筆誤,已依意圖寫入(美食分頁本就明載全版本共用)。
- 規格書版本號 → v2.1(2026-07-06)。jsdom 測試 24 項全過(既有 23＋新增英文序數 1)。
